...
namespace MI {
	...
	public:
		...
		Vector(const T* dat, size_t len) : capElements{ len }, numElements{ len }, data{ new T[len] } {
			pcpy(data, len, dat);
		}
		Vector(const T* dat, size_t len, size_t cap) : capElements{ cap }, numElements{ len }, data{ new T[cap] } {
			pcpy(data, len, dat);
		}
