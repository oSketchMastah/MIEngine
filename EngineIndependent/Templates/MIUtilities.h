#pragma once
#include <Memory>//literally just for std::move (try and do something about it later)

namespace MI {
	//copy NumToCopy elements from the memory location at src to that of dest
	template <typename T>
	void inline pcpy(T* dest, size_t NumToCopy, T* src) noexcept {
		for (; NumToCopy > 0; NumToCopy--, dest++, src++) { *dest = *src; }
	}
	
	//advance the pointers at src and dest while copying data from src to dest
	template <typename T>
	void inline pbuildcpy(T*& dest, size_t NumToCopy, T*& src) noexcept {
		for (; NumToCopy > 0; NumToCopy--, dest++, src++) { *dest = *src; }
	}
	
	//advance the src pointer, keep the 
	template <typename T>
	void inline pparsecpy(T* dest, size_t NumToCopy, T*& src) noexcept {
		for (; NumToCopy > 0; NumToCopy--, dest++, src++) { *dest = *src; }
	}
